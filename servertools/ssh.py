...
class SFTPHandler:
	...
	def __init__(self, server, wrkpath=None):
	...
	def put_file(self, localpath, remotepath, safe=False):
		...
		if not rdir:
			rdir = "."
		if remotepath in self.sftp.listdir(rdir):
	...
	def put_dirs(self, localpath, remotepath, safe=False):
		...
		if  not os.path.isdir(localpath):
			self.put_file(localpath, remotepath+"/"+localpath)
			...
			for item in os.listdir(localpath):
				...
				if os.path.isdir(item):
					...
					try:
						...
						pass
					self.put_dirs(os.path.join(localpath, item), remotepath+"/"+item)
				else: #we have files here
					self.put_file(os.path.join(localpath, item), remotepath+"/"+item)
